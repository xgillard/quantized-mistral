//! The content of this file was mostly copied from candle/examples.
//! The only modifications that were brought to this file are the following:
//! - i dropped the from_ggml which i did not needed
//! - i dropped some tracing info as i did not need them either (and i did not want to take an additional dependency)
//! - i added a method forward_with_mask which can be used to prime the model with a long prompt much faster.
//!   => priming the model without a mask is actually not a problem: the prompt is a given when the point is to generate text.
//!      therefore the model should have the right to access it all (+ it avoids a broadcasting issue)
...
impl ModelWeights {
    ...
    pub fn forward_with_mask(&mut self, x: &Tensor, index_pos: usize, mask: Option<Tensor>) -> Result<Tensor> {
        let (_b_sz, seq_len) = x.dims2()?;
        let mut layer_in = self.tok_embeddings.forward(x)?;
        for layer in self.layers.iter_mut() {
            let x = layer_in;
